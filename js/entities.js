...
const NPC_DEFS = [
  ...
  { id: 'fisher', name: 'Old Salt Pernn', icon: '🎣', x: LOCS.saltwind.x + 4, y: LOCS.saltwind.y + 3, wander: 0 },
  // --- conditional / hidden ---
...
const DIALOGS = {
  ...
  fisher() {
    return node('"' + pick(['Caught a boot today. Yesterday, two boots. The sea provides... footwear.', 'See that island out past the dock? Folk who row there come back rich. Or not at all. Mostly the second one.', 'Pearls in these shells, if you\'ve the patience. I haven\'t, but YOU might.']) + '"', bye());
  },
};
