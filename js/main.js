...
const Game = {
  ...
  // if the hero ended up inside a solid tile (e.g. an old save on an updated map),
  // spiral outward to the nearest walkable tile so they're never trapped
  unstick() {
    const p = G.player;
    if (p.onBoat) return; // sailing on water is fine
    if (!isBlocked(p.x, p.y) && OPEN_GROUND.has(tileAt(p.x, p.y))) return;
    for (let r = 1; r <= 12; r++) {
      for (let dy = -r; dy <= r; dy++) for (let dx = -r; dx <= r; dx++) {
        if (Math.abs(dx) !== r && Math.abs(dy) !== r) continue; // ring only
        const nx = p.x + dx, ny = p.y + dy;
        if (!isBlocked(nx, ny) && OPEN_GROUND.has(tileAt(nx, ny))) {
          p.x = nx; p.y = ny; p.px = nx; p.py = ny;
          return;
        }
      }
    }
    // last resort: back to the village spawn
    if (!G.inDungeon && !G.inInterior) { p.x = LOCS.spawn.x; p.y = LOCS.spawn.y; p.px = p.x; p.py = p.y; }
  },
  ...
  newGamePlus() {
    ...
    G.inInterior = false; G.interior = null; G.interiorHouse = null;
    G.houses = []; G.boatPos = null; G.player.onBoat = false; // fresh world
  ...
  updateMovement(dt) {
    ...
    if (mx || my) {
      ...
      if (this.canMoveTo(nx, ny)) { this.boatTransition(nx, ny); p.x = nx; p.y = ny; p.moving = true; }
  ...
  // movement permission including the boat (sail on water, board a docked boat)
  canMoveTo(nx, ny) {
    const p = G.player;
    if (p.onBoat) return isWater(nx, ny) || !isBlocked(nx, ny); // sail water, or step ashore
    if (G.boatPos && nx === G.boatPos.x && ny === G.boatPos.y) return true; // board the boat
    return !isBlocked(nx, ny);
  },

  // handle boarding / sailing / disembarking as the hero changes tiles
  boatTransition(nx, ny) {
    const p = G.player;
    if (!p.onBoat) {
      if (G.boatPos && nx === G.boatPos.x && ny === G.boatPos.y) {
        p.onBoat = true; G.boatPos = { x: nx, y: ny };
        Audio2.sfx('flee'); FX.float(nx, ny - 0.5, '⛵ Wheee!', '#5fa6d8');
        UI.notify('⛵ You set sail! Steer with WASD; step ashore to dock.', 'good');
      }
      return;
    }
    // on the boat
    if (isWater(nx, ny)) { G.boatPos = { x: nx, y: ny }; FX.spark(p.x, p.y + 0.3, '#bfe6ff'); }
    else { // stepping onto land → leave the boat bobbing at the water tile we were on
      G.boatPos = { x: p.x, y: p.y }; p.onBoat = false;
      Audio2.sfx('step'); UI.notify('You hop ashore. Your boat waits at the water\'s edge.', '');
    }
  },
