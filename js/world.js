...
const LOCS = {
  ...
  lairGate: { x: 168, y: 37 },        // obsidian door to dragon's lair (on the entrance corridor)
...
function genWorld(seed) {
  ...
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      if (x < 2 || y < 2 || x > W - 3 || y > H - 3) { set(x, y, T.OCEAN); setZ(x, y, 'coast'); continue; }
      ...
      switch (biome) {
        ...
        case 'snow':
          t = d > 0.66 ? T.SNOWTREE : (d2 < 0.08 ? T.ICE : (d > 0.78 ? T.ROCK : T.SNOW)); break;
        case 'desert':
          t = d > 0.74 ? T.CACTUS : (d > 0.5 ? T.DUNE : (d2 < 0.03 ? T.ROCK : T.SAND)); break;
        case 'swamp':
          t = d > 0.66 ? T.DEADTREE : (d < 0.34 ? T.SWAMPWATER : T.SWAMP); break;
        case 'highland':
          t = d > 0.84 ? T.MOUNTAIN : (d > 0.7 ? T.ROCK : (d2 > 0.85 ? T.TREE : T.HIGHGRASS)); break;
        case 'volcanic':
          t = d < 0.28 ? T.LAVA : (d > 0.82 ? T.ROCK : T.ASH); break;
        case 'cursed':
          t = d > 0.6 ? T.CURSEDTREE : (d2 < 0.04 ? T.ROCK : T.CURSEDGROUND); break;
        case 'ruins':
          t = d > 0.82 ? T.PILLAR : (d > 0.6 ? T.RUINFLOOR : (d2 > 0.9 ? T.TREE : T.GRASS)); break;
        case 'coast':
          if (dist > 80) { t = T.OCEAN; } else { t = d2 < 0.4 ? T.SAND : T.GRASS; }
          break;
      }
      // the whole map is ringed by sea, with a sandy beach just inside it
      if (dist > 90) { t = T.OCEAN; zone = 'coast'; }
      else if (dist > 85 && t !== T.OCEAN) { t = T.SAND; zone = 'coast'; }
  ...
  // single gated entrance through the south wall (y37), then a clear approach corridor
  for (let y = 37; y <= 48; y++) {
    for (let x = 167; x <= 169; x++) {
      if (y === 37) { if (x === 168) { set(x, y, T.GATE); setZ(x, y, 'lair'); } continue; } // gate is the only opening in the wall
      set(x, y, x === 168 ? T.LAIRFLOOR : T.ASH); setZ(x, y, 'lair');
    }
  }
  ...
  // Hidden island (SE ocean) — a treasure isle
  const isl = LOCS.island;
  blob(isl.x, isl.y, 7, T.SAND, 'island');
  blob(isl.x, isl.y, 4, T.GRASS, 'island');
  set(isl.x, isl.y, T.SHRINE);
  for (const [dx, dy] of [[-3, -3], [3, 3], [-3, 3], [3, -3], [0, -4], [4, 0]]) set(isl.x + dx, isl.y + dy, T.TREE);
  // scattered treasure chests
  for (const [dx, dy] of [[-2, -1], [2, -1], [-2, 2], [2, 2], [-4, 1], [4, 1], [0, 3], [-1, -3], [1, 3]])
    if (!SOLID.has(get(isl.x + dx, isl.y + dy))) set(isl.x + dx, isl.y + dy, T.CHEST);
  ...
  carvePath([[100, 50], [150, 45], [168, 47]]);  // → Dragon's Lair approach corridor
...
function isWater(x, y) { const t = tileAt(x, y); return t === T.OCEAN || t === T.WATER; }
