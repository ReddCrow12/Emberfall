...
const House = {
  ...
  beginBoat(item) {
    if (G.inInterior || G.inDungeon) { UI.notify('You need open water to launch a boat.', 'bad'); return; }
    UI.closePanel();
    Game.pointerActive = false;
    Game.placing = { kind: 'boat', item, gx: G.player.x, gy: G.player.y, valid: false };
    this.updateGhost(G.player.x, G.player.y - 1);
    UI.showPlacementBar('Launching the boat — pick an adjacent water tile, then tap to drop it in.');
  },
  ...
  updateGhost(tx, ty) {
    ...
    } else if (pl.kind === 'boat') {
      pl.gx = tx; pl.gy = ty;
      // launch only onto water within reach of the shore you're standing on
      pl.valid = isWater(tx, ty) && (Math.abs(tx - G.player.x) + Math.abs(ty - G.player.y) <= 2) &&
        !(G.boatPos && G.boatPos.x === tx && G.boatPos.y === ty);
    } else {
      pl.gx = tx; pl.gy = ty;
  ...
  confirm() {
    ...
    if (pl.kind === 'house') {
      ...
      G.boatPos = { x: pl.gx, y: pl.gy };
      G.player.onBoat = false;
      removeItem(pl.item, 1);
      Audio2.sfx('chest');
      FX.burst(pl.gx, pl.gy, '#5fa6d8', 16);
      UI.notify('🛶 Boat launched! Step onto it to set sail.', 'good');
    } else {
